...
class TextScrambler {
  ...
  constructor(el, config = {}) {
    ...
    const defaults = {
      ...
      autoInitialize: true, // If true, automatically set initial state based on reverse mode
    }
    ...
    // Auto-initialize display state based on reverse mode
  ...
  _setInitialState() {
    if (this.config.reverse) {
      // Reverse mode: start with original text visible
      this._updateTextNodes(this.originalText)
    } else {
      // Forward mode: start with masked text (spaces)
      this._updateTextNodes(this.mask)
    }
  }

  /**
   * Reinitializes the animation state.
   * @returns {TextScrambler} The instance.
   */
  initialize() {
    this.currentMask = this.mask
    if (this.config.autoInitialize) {
    ...
    if (Math.random() < 0.5 && progress > 0 && progress < 1) {
      for (let t = 0; t < 20; t++) {
        ...
        if (this.config.reverse) {
          ...
          idx = revealCount - Math.floor((1 - Math.random()) * offset * factor)
        } else {
        ...
        if (this.currentMask[idx] !== ' ') {
          const newChar =
            Math.random() > this.config.randomThreshold
              ...
              : getRandomScrambleChar(
                  this.config.reverse,
                  this.config.scrambleSymbols
                )
  ...
  _tick = timestamp => {
    ...
    const offset = Math.floor(
      2 * (0.5 - Math.abs(progress - 0.5)) * this.scrambleRange
    )
